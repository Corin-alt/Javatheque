def deployToEnvironment(String deployServer, String sshKey) {
    sh '''
    ...
    sshagent(credentials: [sshKey]) {
        ...
        mkdir -p ~/.ssh
        chmod 700 ~/.ssh
        TARGET_IP=\$(echo ${deployServer} | cut -d'@' -f2)

        ssh-keyscan -H \$TARGET_IP >> ~/.ssh/known_hosts
        chmod 644 ~/.ssh/known_hosts

        # Synchronisation des fichiers
        rsync -av --delete --exclude=${DB_VOLUME} ./ ${deployServer}:${APP_PATH}/
        scp target/${APP_NAME}.war ${deployServer}:${APP_PATH}/target

        # Configuration environnement
        ssh ${deployServer} "printf 'DOCKER_REGISTRY=%s\\nGITHUB_OWNER=%s\\nDOCKER_IMAGE=%s\\nDOCKER_TAG=%s\\nAPP_DEPLOY_PATH=%s' '${DOCKER_REGISTRY}' '${GITHUB_OWNER}' '${DOCKER_IMAGE}' '${DOCKER_TAG}' '${APP_DEPLOY_PATH}' > ${APP_PATH}/.env"

        # Déploiement Docker
        ssh ${deployServer} "cd ${APP_PATH} && \
...
pipeline {
    ...
    stages {
        ...
        stage('Deploy to Production') {
            ...
            steps {
                script {
                    deployToEnvironment(env.DEPLOY_PROD_SERVER, 'jenkins-ssh-private-key')
                }
            }
        }
    }
